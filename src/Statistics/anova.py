...
class UIANOVA(QWidget):
    def __init__(self, ui):
        super().__init__()
        self.ui = ui
        self.ui.actionANOVA.triggered.connect(self.run)
    ...
    def run_ttest(self):
        grouped = [group['autocorr'].values for name, group in self.cell_data.groupby('condition')]
        ...
        if p < 0.05:
            tukey = pairwise_tukeyhsd(endog=self.cell_data['autocorr'],
                          groups=self.cell_data['condition'],
                          alpha=0.05)
            
            data = DataModel(pd.DataFrame(data=tukey._results_table.data[1:], columns=tukey._results_table.data[0]))
        else:
